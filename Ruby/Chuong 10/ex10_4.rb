...
time = Time.new
filename = "./Chuong 10/" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".json"
  
hash_all = {}
  if arr_vang.size > 0
    hash_vang = Array.new()
    arr_vang.each do |vang|
      hash_vang<<object_to_hash(vang)
    end
    hash_all["giao_dich_vang"] = hash_vang
  end
  
  if arr_tien.size > 0
    hash_tien = Array.new()
    arr_tien.each do |tien|
      hash_tien<<object_to_hash(tien)
    end
    hash_all["giao_dich_tien"] = hash_tien
  end
  
  if hash_all.size > 0
    File.write(filename,hash_all.to_json)
    puts("Đã ghi nội dung vào #{filename}")
  end
